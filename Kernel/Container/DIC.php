...
class DIC implements ContainerInterface
{
    ...
    private function __construct(bool $automatisation = true)
    {
        $this->automatisation = $automatisation;
    }

    public static function buildContainer(bool $automatisation = true): self
    {
        if (self::$DIC == null) {
    ...
    public function addDefinitions(string $path)
    {
        $arrayConfig = require $path;
        foreach ($arrayConfig as $id => $callback) {
