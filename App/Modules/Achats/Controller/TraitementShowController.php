...
class TraitementShowController extends AbstractTraitementShowController
{
    ...
    public function process(ServerRequestInterface $request, \Psr\Http\Server\RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        switch ($action) {
            ...
            case "modifier":
                return $this->modifier($id, "@AchatsTraitement/modifier_form");
                break;

            case "ajouter":
                $getInfo = $this->getRequest()->getQueryParams();
                if (!isset($getInfo["ajouter"])) {
                    $response = $this->ajouter_select("@AchatsTraitement/ajouter_select");
                    if ($response !== null) {
                        return $response;
                    } else {
                        // if table is mastre (table premier )
                        return $this->ajouter($getInfo, "@AchatsTraitement/ajouter_form");
                    }
                } else {
                    return $this->ajouter($getInfo, "@AchatsTraitement/ajouter_form");
                }
            ...
            case "voir":
                return $this->show($id, "@AchatsShow/show_id");
                break;

            case "message":
                return $this->message($id, "@AchatsShow/show_message_id");
                break;
