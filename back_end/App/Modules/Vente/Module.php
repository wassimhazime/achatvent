...
class Module {
    ...
    public function addPathRenderer($renderer, $pathModules) {
        $renderer->addPath($pathModules . "Vente" . D_S . "views", "vente");
    }

    public function addRoute($router) {

        // $router->get("/{controle:[a-z]+}", [$this, "MVC"], "blog.strj");
        $router->get("/vente/{controle:[a-z\_\-\$]+}", [$this, "MVC"], "vente.get");
        

        $router->post("/vente/{controle:[a-z\_\-\$]+}", [$this, "POST"], "vente.post");
